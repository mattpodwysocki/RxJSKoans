...
(function (root, factory) {
    ...
    observableProto.aggregate = function () {
        ...
        return hasSeed ?
            this.scan(seed, accumulator)
                .startWith(seed)
                .finalValue() :
            this.scan(accumulator)
                .finalValue();
    };
    ...
    observableProto.standardDeviation = function (keySelector) {
        return keySelector ?
            this.select(keySelector).standardDeviation() :
            this.aggregate({
                sumSq: 0,
                sum: 0,
                count: 0
    ...
    // TODO: should standardDeviation just be Math.sqrt of variance?
    observableProto.variance = function (keySelector) {
        return keySelector ?
            this.select(keySelector).variance() :
            this.aggregate({
                sumSq: 0,
                sum: 0,
                count: 0
    ...
    observableProto.tally = function (selector) {
        return selector ?
            this.select(selector).tally() :
            this.aggregate(
                {},
                function(cume, curr)

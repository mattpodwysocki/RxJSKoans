...
(function (window, undefined) {
    ...
    var Enumerable = root.Enumerable = (function () {
        ...
        EnumerablePrototype.standardDeviation = function(selector) {
            if (selector) {
                return this.select(selector).standardDeviation();
            }
            var enumerator = this.getEnumerator(), count = 0, sumSq = 0, sum = 0;
            var current;
            try {
                while (enumerator.moveNext()) {
                    count++;
        ...
        // TODO: determine whether this should be merged with standardDeviation,
        // i.e., should standardDeviation just be Math.sqrt(variance)?
        EnumerablePrototype.variance = function(selector) {
            if (selector) {
        ...
        EnumerablePrototype.tally = function(selector)
        {   if (selector)
                return this.select(selector).tally();
            
            var enumerator = this.getEnumerator(), cume = {};
            var curr;
            try {
                while (enumerator.moveNext()) {
                    curr = enumerator.getCurrent();
                    cume[curr] = (cume[curr] || 0) + 1;
                }
            } finally {
                enumerator.dispose();
            }
    ...
    var enumerableFromObject = Enumerable.fromObject = function(object)
    {   return new Enumerable(function ()
        {   var key, value;
